...
namespace Snake {
    SnakeGameOverScene::SnakeGameOverScene() : SnakeScene()
    {
    }
    ...
    bool SnakeGameOverScene::init()
    {
        ...
        _systemManager->addSystem("BufferWriterManager", std::make_unique<Snake::BufferWriterManager>());

        auto &entity = _entityManager->createEntity("Text");
        auto &entity2 = _entityManager->createEntity("BufferText");

        auto text = std::shared_ptr<Snake::Text>(new Snake::Text("Text",
            "Enter your username and press enter to save your score", FONT_PATH, { 0, 0, 0, 255 }, { 255, 255, 255, 255 }, { BASE_X, 50, 0 }));
        text->pos.y = OFFSET_Y_TEXT;

        auto text2 = std::shared_ptr<Snake::Text>(new Snake::Text("Text",
